...
$NinjaOneClientSecret = "fR2mUs7KjPhy4DJkWhHccm2X7hdLnl1D5yRtCfwC_xRVA9oWnG2Pfg"
...
while ($hasMoreData) {
    ...
    # Build the URL
    if ($null -ne $cursor) {
        $url = "https://eu.ninjarmm.com/v2/queries/os-patch-installs?cursor=$cursor&pageSize=200"
    } else {
        $url = "https://eu.ninjarmm.com/v2/queries/os-patch-installs?page_size=200"
    }
